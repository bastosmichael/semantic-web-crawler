...
unsigned int PcreWrapper::DeterminePcreOptions ( const std::string & opts ///< perl style character modifiers -- i.e. "gi" is global, case-insensitive
 )
{
...
void					
PcreWrapper::options(unsigned opts ///< PCRE-style option flags to be set on PcreWrapper object
)
{
	_opts = opts;
}
...
void 
PcreWrapper::compile(const std::string & s ///< string to compile into regular expression
)
{
	const char * errorptr;
	int erroroffset;
...
pcre *
PcreWrapper::clone_re(pcre * re ///< PcreWrapper object to clone
)
{
	if ( !re )
		return 0;
...
PcreWrapper::PcreWrapper(const std::string & s ///< string to copmile into regular expression
			 , unsigned opts ///< PCRE-style option flags to be set on PcreWrapper object
)
{
	reset ( );
	m_isglobal = 0;
...
PcreWrapper::PcreWrapper ( const std::string & s, ///< string to compile into regular expression
		   const std::string & opts ///< perl-style character flags to be set on PcreWrapper object 
)
{
	reset ( );
	m_isglobal = 0;
...
PcreWrapper::PcreWrapper(const char * s, ///< string to compile into regular expression
			 unsigned opts ///< PCRE-style option flags to be set on PcreWrapper object
)
{
	reset ( );
...
PcreWrapper::PcreWrapper(const PcreWrapper & r ///< PcreWrapper object to make copy of 
)
{
	reset ( );
	m_isglobal = 0;
...
int
PcreWrapper::match(const std::string & s, ///< s String to match against
...
//		fprintf ( stderr, "PcreWrapper RESETTING: new string\n" );
		lastglobalposition = 0;
	}
...
//			fprintf ( stderr, "PcreWrapper RESETTING: reset for no match\n" );
			lastglobalposition = 0; // reset the position for next match (perl does this)
		} else if ( nMatches > 0 ) {
//			fprintf ( stderr, "PcreWrapper RESETTING: setting to %d\n", marks[0].second );
			lastglobalposition = marks[0].second; // increment by the end of the match
		} else {
//			fprintf ( stderr, "PcreWrapper RESETTING: reset for no unknown\n" );
			lastglobalposition = 0;
		}
	}
...
std::string
PcreWrapper::substr(const std::string & s, const vector<PcreWrapper::markers> & marks,
			  unsigned index)
{
 ...
 */
int
PcreWrapper::split(const std::string & s, unsigned maxfields)
{
...
std::string  PcreWrapper::GetSubStr(int index)
{   
    	return substr ( laststringmatched, m_marks, index );
}

std::string PcreWrapper::operator[](int index)
{
...
std::string PcreWrapper::UpdateReplacementString ( const std::string & r ) {
	std::string finalreplacement = r;
	
	// search for each backref and store it out
	PcreWrapper dollars ( "\\$([0-9]+)", "g" );
	...
	while ( int numdollars = dollars.match ( r ) ) {
		...
		regextext << "[$]" << dollars[1];
		PcreWrapper dollarsub ( regextext.str ( ) );
...
std::string PcreWrapper::sub ( const std::string & s, const std::string & r,
...
StringVector PcreWrapper::GetStringVector ( ) 
{
